...
class BestRQBrain(sb.core.Brain):
    ...
    def on_fit_batch_end(self, batch, outputs, loss, should_step):
        ...
        if  (self.step < self.growth_limit) and (self.step % self.growth_interval) == 0 and self.optimizer_step > 0:
            ...
            print(f'updated growth stage to {self.growth_stage}')
            # print(self.hparams.modules['wrapper'])
            self.hparams.modules['wrapper'].transformer.encoder.layers_to_use = self.total_layers[:self.growth_stage] + self.total_layers[-self.growth_stage:]
            print('now using layers ', self.hparams.modules['wrapper'].transformer.encoder.layers_to_use)

            layer_0_weights_changed = has_weights_changed(self.initial_weights_layer_0, self.hparams.modules['wrapper'].transformer.encoder.layers[0].parameters())
            layer_1_weights_changed = has_weights_changed(self.initial_weights_layer_1, self.hparams.modules['wrapper'].transformer.encoder.layers[1].parameters())
            layer_2_weights_changed = has_weights_changed(self.initial_weights_layer_2, self.hparams.modules['wrapper'].transformer.encoder.layers[2].parameters())
            layer_3_weights_changed = has_weights_changed(self.initial_weights_layer_3, self.hparams.modules['wrapper'].transformer.encoder.layers[3].parameters())

            print(f"Layer 0 weights have changed: {layer_0_weights_changed}")
            print(f"Layer 1 weights have changed: {layer_1_weights_changed}")
            print(f"Layer 2 weights have changed: {layer_2_weights_changed}")
            print(f"Layer 3 weights have changed: {layer_3_weights_changed}")
...
def main():
    ...
    brain.growth_limit = hparams['growth_interval'] * brain.modules.wrapper.transformer.encoder.num_layers / 2 * brain.grad_accumulation_factor
    brain.growth_interval = hparams['growth_interval'] * brain.grad_accumulation_factor
    print(f'brain.growth_stage: {brain.growth_stage}')
    print(f'brain.growth_interval: {brain.growth_interval}')
    brain.total_layers = list(range(brain.modules.wrapper.transformer.encoder.num_layers))
    print(f'brain.total_layers: {brain.total_layers}')
    # brain.modules.wrapper.transformer.encoder.layers_to_use = [0,11]
    ...
    brain.initial_weights_layer_0 = [p.clone() for p in brain.modules.wrapper.transformer.encoder.layers[0].parameters()]
    brain.initial_weights_layer_1 = [p.clone() for p in brain.modules.wrapper.transformer.encoder.layers[1].parameters()]
    brain.initial_weights_layer_2 = [p.clone() for p in brain.modules.wrapper.transformer.encoder.layers[2].parameters()]
    brain.initial_weights_layer_3 = [p.clone() for p in brain.modules.wrapper.transformer.encoder.layers[3].parameters()]
...
def has_weights_changed(initial_weights, current_parameters):
    for initial, current in zip(initial_weights, current_parameters):
