...
class BestRQBrain(sb.core.Brain):
    ...
    def on_stage_start(self, stage, epoch):
        ...
        # if training and told to reset and step is a multiple of 100 -> reset
        # a bit hacky: the reason for checking if step is multiple of 100 if code crashed and we
        # want to train using a checkpoint, we would not want to reset the optimizer
        # basically we are assuming that it is very unlikely for an epoch to finish on a multiple of 100
        if stage == sb.Stage.TRAIN and self.reset_optimizer and (self.optimizer_step % 100 == 0):
        ...
        if stage == sb.Stage.TRAIN and self.init_using_previous_layers and self.growth_stage > 1 and (self.optimizer_step % 100 == 0):
            target_layer_idx = self.growth_stage - 1
            source_layer_idx = self.growth_stage - 2
            print(f"initiliazing layer indx: {target_layer_idx} with {source_layer_idx}")
            self.modules.wrapper.module.transformer.encoder.reinitialize_layer_using_layer(target_layer_idx, source_layer_idx, noise_std=self.init_noise)
...
def main():
    ...
    # define how to initialize new layers
    if hparams['init_using_previous_layers']:
        print("setting init_using_previous_layers to True")
        brain.init_using_previous_layers = True
        print("setting noise")
        brain.init_noise = hparams['init_noise']
        print(brain.init_noise)
    else:
         brain.init_using_previous_layers = False
